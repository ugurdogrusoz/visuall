...
### Maintenance
If you forked a new repository from Visu*all*, you might want to get updates from [the original Visuall repository](https://github.com/ugurdogrusoz/visuall). Sometimes there might be some improvements or bug fixes. To do that, first add Visu*all* with an alias as a remote repo. `git remote add base https://github.com/ugurdogrusoz/visuall.git`. In the previous commond we added Visu*all* with name "base". Then fetch commits of Visu*all* with `git fetch base`. Then transfer changes of commits using a command such as `git cherry-pick fe75f -n`. Here "fe75f" refers to ID of a commit in Visu*all*.
